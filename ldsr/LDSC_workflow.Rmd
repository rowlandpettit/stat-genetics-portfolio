...
Use baseline annotations (e.g., baselineLD v2.2). Create filtered vs. unfiltered sumstats to exclude ±500kb around smoking loci, then compare enrichments.

```bash
echo "Using baselineLD v2.2"
...
python $LDSC/ldsc.py \
  ...
  --overlap-annot \
  --frqfile-chr $REF/ \
  --out trait1_partitioned_unfiltered

# Filtered (after removing smoking windows → trait1_filtered.munged.sumstats.gz)
python $LDSC/ldsc.py \
  --h2 trait1_filtered.munged.sumstats.gz \
  --ref-ld-chr ${BASE} \
  --w-ld-chr $REF/ \
  --overlap-annot \
  --frqfile-chr $REF/ \
  --out trait1_partitioned_filtered
```
...
part_unf <- read_delim("trait1_partitioned_unfiltered.results", delim = "\t")
part_fil <- read_delim("trait1_partitioned_filtered.results", delim = "\t")
comparison <- part_unf %>% select(Category, Enrichment_unf = Enrichment) %>%
  left_join(part_fil %>% select(Category, Enrichment_fil = Enrichment), by = "Category") %>%
  mutate(Delta = Enrichment_fil - Enrichment_unf) %>% arrange(desc(abs(Delta)))
comparison %>% head(10)
write_csv(comparison, "ldsc_partitioned_enrichment_comparison.csv")
```
